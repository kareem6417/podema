...
// Ambil status dari URL. Defaultnya adalah 'active'.
...
if ($task_status == 'active') {
    // Tugas yang BELUM selesai, dijamin TIDAK menampilkan Completed
    $status_clause = " AND ji.status_jadwal IN ('Pending', 'In Progress')";
    $task_title = "Tugas Inspeksi Aktif";
} elseif ($task_status == 'completed') {
    // Tugas yang SUDAH selesai, HANYA menampilkan Completed
    // Kita filter berdasarkan NIK pelaksana IT yang login
...
// Query untuk mengambil daftar tugas
$sql_tasks = "
    SELECT 
        ji.jadwal_id,
        ji.tanggal_jadwal,
        ji.jenis_perangkat,
        ji.status_jadwal,
        ji.id_hasil_inspeksi, -- PENTING untuk link ke viewinspeksi
        ma.merk,
        ma.serial_number,
    ...
    LEFT JOIN
        users staf ON ji.id_staf_it = staf.nik -- JOIN untuk menampilkan nama staf IT
    WHERE
        ji.jenis_perangkat = ma.jenis 
        " . $status_clause . "
    ORDER BY 
...
// Bind parameter :nik hanya jika filter 'completed' aktif
// Catatan: Jika tugas aktif tidak di-filter oleh NIK, logika ini harus diubah.
// Asumsi: Tugas aktif dan selesai hanya milik staf IT yang login (sesuai NIK)
// Jika Anda ingin menampilkan SEMUA tugas Pending (unassigned), hapus ":nik" dari klausa dan $task_params.
if ($task_status == 'completed') {
    $stmt_tasks->execute($task_params);
} else {
    // Menampilkan semua tugas aktif (Pending/In Progress), TIDAK filter oleh NIK
    // Jika filter berdasarkan NIK diperlukan, ubah $status_clause
    $stmt_tasks->execute();
}
                            ...
                            <?php if (count($tasks) > 0): ?>
                                <?php $no = 1; foreach ($tasks as $task): 
                                    ...
                                    if ($task['status_jadwal'] == 'Completed') {
                                        ...
                                        // Asumsi penamaan file inspeksi adalah ins_[jenis_perangkat].php
                                        $jenis_perangkat_clean = strtolower(str_replace(' ', '_', $task['jenis_perangkat']));
                                        $aksi_link = "ins_" . $jenis_perangkat_clean . ".php?jadwal_id=" . $task['jadwal_id'] . "&aset_id=" . $task['aset_id'];
                                        $aksi_button_class = "btn-success";
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="8" class="text-center">
                                        <?php echo ($task_status == 'active' ? 'Tidak ada tugas inspeksi aktif yang ditemukan.' : 'Tidak ada riwayat inspeksi selesai yang ditemukan.'); ?>
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
