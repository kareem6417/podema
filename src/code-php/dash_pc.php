...
$params = [];
$where_clauses = "WHERE 1=1"; // Base clause

$count_stmt = $conn->prepare("SELECT COUNT(*) FROM assess_pc " . $where_clauses);
...
$main_sql = "SELECT ap.*, 
                pt.pctype_name, os.os_name, proc.processor_name, vga.vga_name, age.age_name, 
                iss.issue_name, ram.ram_name, store.storage_name, tm.monitor_name, sm.size_name
            FROM assess_pc ap
            LEFT JOIN pctype_pc pt ON ap.typepc = pt.pctype_score
            LEFT JOIN operating_sistem_pc os ON ap.os = os.os_score
            LEFT JOIN processor_pc proc ON ap.processor = proc.processor_score
            LEFT JOIN vga_pc vga ON ap.vga = vga.vga_score
            LEFT JOIN device_age_pc age ON ap.age = age.age_score
            LEFT JOIN issue_software_pc iss ON ap.issue = iss.issue_score
            LEFT JOIN ram_pc ram ON ap.ram = ram.ram_score
            LEFT JOIN storage_pc store ON ap.storage = store.storage_score
            LEFT JOIN typemonitor_pc tm ON ap.typemonitor = tm.monitor_score
            LEFT JOIN sizemonitor_pc sm ON ap.sizemonitor = sm.size_score " . 
            $where_clauses . " ORDER BY ap.date DESC, ap.id DESC LIMIT :limit OFFSET :offset";
...
<head>
  ...
  <title>Dashboard PC Desktop Assessment</title>
  ...
  <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full" data-sidebar-position="fixed" data-header-position="fixed">
    ...
    <div class="body-wrapper">
      ...
      <div class="container-fluid">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title fw-semibold mb-4">Dashboard PC Desktop Assessment</h5>
                        ...
                        <tr>
                            ...
                            <td class="action-icons">
                                <span data-bs-toggle="modal" data-bs-target="#detailModal" 
                                      ...
                                      data-typepc="<?php echo format_detail_string($row['pctype_name'], $row['typepc']); ?>"
                                      ...
                                      data-typemonitor="<?php echo format_detail_string($row['monitor_name'], $row['typemonitor']); ?>"
                                      data-sizemonitor="<?php echo format_detail_string($row['size_name'], $row['sizemonitor']); ?>"
                                      data-score="<?php echo htmlspecialchars($row['score']); ?>">
                                    <i class="ti ti-eye text-primary" title="View Details"></i>
                                </span>
                                <a href="./download_pc_assessment.php?id=<?php echo $row['id']; ?>" target="_blank">
            ...
            <nav aria-label="Page navigation" class="mt-4 d-flex justify-content-end">
                <ul class="pagination">
                    <?php if ($currentPage > 1): ?>
                        <li class="page-item"><a class="page-link" href="?page=<?php echo $currentPage - 1; ?>&limit=<?php echo $limit; ?>">Previous</a></li>
                    <?php endif; ?>
                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                        <li class="page-item <?php if ($i == $currentPage) echo 'active'; ?>"><a class="page-link" href="?page=<?php echo $i; ?>&limit=<?php echo $limit; ?>"><?php echo $i; ?></a></li>
                    <?php endfor; ?>
                    <?php if ($currentPage < $totalPages): ?>
                        <li class="page-item"><a class="page-link" href="?page=<?php echo $currentPage + 1; ?>&limit=<?php echo $limit; ?>">Next</a></li>
                    <?php endif; ?>
                </ul>
            </nav>
          </div>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="detailModalLabel">Detail PC Desktop Assessment</h5>
        ...
        <div class="modal-body">
            <table class="table table-bordered">
                ...
                <tr><td>PC Type</td><td id="modal-typepc"></td></tr>
                ...
                <tr><td>Monitor Type</td><td id="modal-typemonitor"></td></tr>
                <tr><td>Monitor Size</td><td id="modal-sizemonitor"></td></tr>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      detailModal.addEventListener('show.bs.modal', function (event) {
          ...
          modalBody.querySelector('#modal-typepc').textContent = triggerElement.getAttribute('data-typepc');
          ...
          modalBody.querySelector('#modal-typemonitor').textContent = triggerElement.getAttribute('data-typemonitor');
          modalBody.querySelector('#modal-sizemonitor').textContent = triggerElement.getAttribute('data-sizemonitor');
